...
void bubble(int* v) {
    ...
    for (i = 0; i < n - 1; i++) //laço externo que percorre até o penúltimo elemento do vetor
        for (j = 1; j < n - i; j++) //laço interno que percorre até o último elemento não ordenado
            if (v[j] < v[j - 1]) { //verifica se o elemento atual é menor que o anterior
...
void merge(int vetor[], int inicio, int meio, int fim) { //combina duas metades de um array que já foram ordenadas
    ...
    while (i <= meio - inicio && j <= fim - inicio) {
        if (temp[i] <= temp[j]) {//compara os elementos das duas metades
...
int main() {
    ...
    do {
        ...
        switch (choice) { // Verifica a escolha do usuário
            case 1:
                srand(time(NULL));
                for(int i=0; i<n; i++){//sorteia os números aleatórios para o array
                    arr[i] = rand();
                }
            ...
            case 5:
                ...
                inicio = clock();//armazena o tempo de início dos clocks do processador
                insertionSort(arr, n); // Chama a função Quick Sort
                fim = clock(); //armazena o tempo de fim dos clocks do processador
            ...
            case 6:
                ...
                inicio = clock();//armazena o tempo de início dos clocks do processador
                insertionSort(arr, n); // Chama a função de Heap Sort
                fim = clock(); //armazena o tempo de fim dos clocks do processador
